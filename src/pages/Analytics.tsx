...
export default function Analytics() {
  ...
  const weeklyActivityData = getLast7Days().map(day => {
    const dayReports = reports.filter(r => r.createdAt.startsWith(day.date)).length;
    const dayTasks = tasks.filter(t => t.createdAt.startsWith(day.date)).length;
    return { ...day, reports: dayReports, tasks: dayTasks };
  });
